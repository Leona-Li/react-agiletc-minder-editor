    ...
    onChange = (e) => {
        ...
        if (value !== '') {
            window.minder.getRoot().traverse((node) => {
                ...
                // let isFind = false;
                nodeResource && nodeResource.map(resource => {
                    if (resource !== null) {
    ...
    render() {
        ...
        const loop = data =>
            data.map(item => {
                ...
                return <TreeNode icon={item.data.id.indexOf('_tag') !== -1 ? <Icon type="tag" /> : null} key={item.data.id} title={text} />;
            });

        return (
            <div style={{ height: '100%' }}>
                <div style={{ margin: '0px 10px' }}>
                    <Input value={searchValue} placeholder="搜索" onChange={this.onChange} />
                </div>
